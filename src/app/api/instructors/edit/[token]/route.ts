...
export async function PATCH(
  ...
  try {
    ...
    const { firstName, lastName, whatsapp, bio, type, levels, mode, city, commune, subjects } = body;
    ...
    if (!firstName || !lastName || !whatsapp || !bio || !bio.trim() || !city || !city.trim() || !commune || !commune.trim() || !Array.isArray(subjects) || subjects.length === 0) {
      return NextResponse.json({ error: 'Champs obligatoires manquants (dont la bio, la ville, la commune et au moins une matière).' }, { status: 400 });
    }
